...
- Исполнитель `ShellTypist` (если доступно расширение): `Backspace(n)` и `Type`
  ...
  используется.
- Предусловие исполнения: ни одна физическая **небуквенная** клавиша не зажата
  (`buffer.held_physical()` содержит только модификаторы или пуста). Если
  зажата — ждём отпускания до 300 мс, иначе план отменяется. Зажатый физический
  Shift снимается шагом `ReleaseModifiers` (в GNOME состояние модификаторов
  общее на seat, синтетическое отпускание его сбрасывает); повторное
  отпускание от физической клавиши позже безвредно.
...
- Инвариант: на любом выходе из исполнения (успех, отмена, ошибка записи в
  ...
  ведёт список нажатых и в `finally` отпускает их в обратном порядке. При
  ошибке записи демон завершает работу с кодом 4 — systemd перезапустит его с
  чистым состоянием.
